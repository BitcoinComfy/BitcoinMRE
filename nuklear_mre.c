...
#if 0
	void
	nk_mre_handle_key_event_old(VMINT event, VMINT keycode){
			...
			switch (keycode) 
			{
				...
				*/
				...
				case VM_KEY_DOWN:
					...
					if (mre_view.hoverable_items > 0)
					{
						if (mre_view.hoverable_counter < mre_view.hoverable_items-1){
...
#if 0
						view = &g_view[view->previous_id];
						if (view->components)
						{
							for(j=0; view->components[j] != NULL; j++){
								if (view->components[j]->type == NK_MRE_COMPONENT_BUTTON &&
									view->components[j]->is_hoverable == 1)
								{
									view->components[j]->hovering = 0;
									b_update_gui = false;
									//break;
								}
							}
...
struct mre_nk_c * mre_nk_c_create(int type, char *title, int len, int hovering, char *url, char *thumb){
	...
	cmpnt = (struct mre_nk_c *)vm_monitored_calloc(sizeof(struct mre_nk_c));
	...
	cmpnt->title = vm_monitored_malloc(strlen(title) + 1);
	strcpy(cmpnt->title, title);
	cmpnt->type = type;
	cmpnt->hovering = hovering;

	if (type == NK_MRE_COMPONENT_BUTTON){
		cmpnt->is_hoverable = 1;
	}
	cmpnt->len = len;
	
	cmpnt->url = vm_monitored_malloc(strlen(url) + 1);
	cmpnt->thumbnail = vm_monitored_malloc(strlen(thumb) + 1);
	strcpy(cmpnt->url, url);
	strcpy(cmpnt->thumbnail, thumb);
	return cmpnt;
}
...
*/
void declare_view_old(struct mre_nk_c** ptr_cmpts){ 
	...
	/* GUI: Start(will allocate mem for context, etc) */
    {
			...
			//NK_WINDOW_SCALABLE|NK_WINDOW_CLOSABLE|NK_WINDOW_MINIMIZABLE
		{
			...
			for(i=0;i<mre_view.components_count;i++){
				...
				if(ptr_cmpts[i]->type == NK_MRE_COMPONENT_BUTTON){
				...
				if(ptr_cmpts[i]->type == NK_MRE_COMPONENT_INPUT_TEXT){
				//if(ptr_cmpts[i].type == "button"){
				...
				if(ptr_cmpts[i]->type == NK_MRE_COMPONENT_TEXT){
				...
				if(ptr_cmpts[i]->type == NK_MRE_COMPONENT_VIDEO){
				//if(ptr_cmpts[i].type == "video"){
					...
					*/
					...
					//image_path = (char*)vm_monitored_malloc(100);
...
void initiate_nk_gui_old(void){
	...
	*/
	...
	font = nk_mrefont_create("Arial", FONT_SIZE);
...
void set_test2_view(){
	...
	struct mre_nk_c **ptr_cmpts = vm_monitored_calloc(10* sizeof(struct mre_nk_c *));
	struct mre_nk_c components[5];
	int i;
	int ptr_cmpts_count;

	b1 = mre_nk_c_create(NK_MRE_COMPONENT_BUTTON, "Mobilizing",0,0,"","");
	b2 = mre_nk_c_create(NK_MRE_COMPONENT_BUTTON, "Quality",0,1,"","");
	b3 = mre_nk_c_create(NK_MRE_COMPONENT_BUTTON, "Education",0,0,"","");
	b4 = mre_nk_c_create(NK_MRE_COMPONENT_BUTTON, "for", 0, 0,"","");
	b5 = mre_nk_c_create(NK_MRE_COMPONENT_BUTTON, "all", 0, 0,"","");
	b6 = mre_nk_c_create(NK_MRE_COMPONENT_BUTTON, "UstadMobile", 0, 0,"","");
...
void set_test_view_here(){
	...
	b1 = mre_nk_c_create(NK_MRE_COMPONENT_BUTTON, "Fox",0,0,"","");
	b2 = mre_nk_c_create(NK_MRE_COMPONENT_BUTTON, "Jumped",0,1,"","");
	b3 = mre_nk_c_create(NK_MRE_COMPONENT_BUTTON, "Dog",0,0,"","");
	t1 = mre_nk_c_create(NK_MRE_COMPONENT_TEXT, "So then he went to the park to think about what just happened and couldn't keep that smile off of his dog's face..",
	...
	v1 = mre_nk_c_create(NK_MRE_COMPONENT_VIDEO, "How to UstadMobile",0,0,"small.mp4","/path/to/video_thumbnail.jpg");
...
////////////////////////////
/*
struct mre_nk_component_t {
	int type;	// component_type
	char *text;
	int is_hoverable;
	int hovering;
	struct nk_color color;
	struct nk_color hovering_color;
};

struct mre_nk_view_t {
	int id;		// enum id
	struct mre_nk_component **components;
	void **view_allocs;
};

//struct mre_nk_component_t mre_nk_component;
//struct mre_nk_view_t mre_nk_view;
*/
...
void update_char_left(VMINT state, VMWSTR text)
{
	if (text != NULL && state == VM_INPUT_OK)
	{
		...
		if (entropy_char_left == 0)
		{
			...
			int type = 0;
			VMWCHAR w_pwd[64];
			uint8_t* loaded_wallet = NULL;
			uint32_t loaded_wallet_len = 0;
			unsigned char seed[] = "123456789 123456789 123456789 123456789 123456789 123456789 123456789 123456789 123456789 123456789 123456789 123456789 123456789 123456789 123456789 123456789 123456789 123456789 123456789 123456789 123456789 123456789 123456789 123456789 123456789 123456789 123456789 123456789 123456789 123456789 123456789 123456789 123456789";
#endif
			// init csprng algo
			...
			vm_ascii_to_ucs2(w_pwd, 64, pwd);
			create_encrypted_wallet("test", TYPE_MNEMONIC, seed, strlen(seed)+1, w_pwd);

			// try correct pass
			loaded = load_encrypted_wallet("test", &type, &loaded_wallet, &loaded_wallet_len, w_pwd);
			...
			// try wrong pass
			vm_ascii_to_ucs2(w_pwd, 64, pwd2);
			loaded = load_encrypted_wallet("test", &type, &loaded_wallet, &loaded_wallet_len, w_pwd);
			if (loaded && loaded_wallet)
			{
				vm_free(loaded_wallet);
