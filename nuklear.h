...
static void* vm_monitored_malloc(int size)
{
	void* alloc = vm_malloc(size);
	printf("vm_malloc(%08X) = %p\n", size, alloc);
	return alloc;
}

static void* vm_monitored_calloc(int size)
{
	void* alloc = vm_calloc(size);
	printf("vm_calloc(%08X) = %p\n", size, alloc);
	return alloc;
}
    ...
    nk_size initial_size)
{
	...
	//b->memory.ptr = a->alloc(a->userdata,0, initial_size); ///bug3 here - solution: aparantely on emulator malloc instead of vm_monitored_malloc works.
	//b->memory.ptr = malloc(initial_size);

	//b->memory.ptr = a->alloc(a->userdata,0, initial_size);
	b->memory.ptr = (void*) vm_monitored_malloc(initial_size); //Doesn't work - Throws exception

	//mem_ptr = (void *) vm_monitored_malloc(initial_size); //Throws Exception
	//mem_ptr = vm_monitored_malloc(initial_size); //Also throws an exception
	//b->memory.ptr = (struct nk_memory*) vm_monitored_malloc(initial_size);// Throws Exception
...
#define NK_COLOR_MAP(NK_COLOR)\
    ...
    NK_COLOR(NK_COLOR_BUTTON_HOVER,         90, 90, 90, 255) \
    NK_COLOR(NK_COLOR_BUTTON_ACTIVE,        85, 85, 85, 255) \
    NK_COLOR(NK_COLOR_TOGGLE,               150, 150, 150, 255) \
    NK_COLOR(NK_COLOR_TOGGLE_HOVER,         120,120,120,255) \
    NK_COLOR(NK_COLOR_TOGGLE_CURSOR,        90, 90, 90, 255) \
    NK_COLOR(NK_COLOR_SELECT,               45, 45, 45, 255) \
    NK_COLOR(NK_COLOR_SELECT_ACTIVE,        85, 85, 85, 255) \
...
nk_style_from_table(struct nk_context *ctx, const struct nk_color *table)
{
    ...
    button->hover           = nk_style_item_color(table[NK_COLOR_BUTTON_HOVER]); // XXX
    button->active          = nk_style_item_color(table[NK_COLOR_BUTTON_ACTIVE]); // XXX
...
#if 0
/* Testing vm_monitored_malloc works until which level or at all here */ 
void test_vm_monitored_malloc(){
	int ptr_size;
	void *ptr;
	ptr_size = 16;
	ptr = vm_monitored_malloc(ptr_size);
	vm_free(ptr);
}
#endif
...
nk_pool_alloc(struct nk_pool *pool)
{
    ...
    if (!pool->pages || pool->pages->size >= pool->capacity) {
        ...
        if (pool->type == NK_BUFFER_FIXED) {
			...
			//ptr_size = 1024;
			//test_ptr = vm_monitored_malloc(ptr_size); // Conclusion: vm_monitored_malloc doesn't work here for some reason
 
			//page = (struct nk_page*)vm_monitored_malloc((int)size); //Exception
			//page = vm_monitored_malloc(size);	//Exception

			//page = (struct nk_page*)pool->alloc.alloc(pool->alloc.userdata,0, size);
			page = (struct nk_page*)vm_monitored_malloc((int)size);
