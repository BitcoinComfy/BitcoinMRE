                     ...
                     const char *dev)
{
    ...
    if((unsigned char)dev[0] < 0x10) {
        ...
        int id = dev[0];
		char vid_str[] = "/dev/video0";
		char* new_alloc = calloc(1, strlen(vid_str)+1);
		strcpy(new_alloc, vid_str);
        dev = ldev = new_alloc;//strdup("/dev/video0");
        ldev[10] = '0' + id;
    }
...
static int video_init_images (zbar_video_t *vdo)
{
    ...
    for(i = 0; i < vdo->num_images; i++) {
        ...
        if(vdo->iomode != VIDEO_MMAP) {
            ...
            offset = i * vdo->datalen;
            img->data = (const void *)((uintptr_t)vdo->buf + offset);
            zprintf(2, "    [%02d] @%08lx\n", i, offset);
        }
    }
    return(0);
