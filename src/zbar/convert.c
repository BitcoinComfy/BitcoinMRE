...
static void uv_roundup (zbar_image_t *img,
                               const zbar_format_def_t *fmt)
{
	unsigned xmask = 0;
	unsigned ymask = 0;
...
static unsigned long uvp_size (const zbar_image_t *img,
                                      const zbar_format_def_t *fmt)
{
    if(fmt->group == ZBAR_FMT_GRAY)
        return(0);
                              ...
                              const zbar_format_def_t *srcfmt)
{
    ...
    if(flags & 1) {
        srcv = (char*)src->data + srcn;
        srcu = srcv + srcm;
    } else {
        srcu = (char*)src->data + srcn;
        srcv = srcu + srcm;
    }
...
static void convert_yuv_unpack (zbar_image_t *dst,
                                ...
                                const zbar_format_def_t *srcfmt)
{
    unsigned long dstn = 0;
    unsigned long dstm2 = 0;
	...
	src->width + (src->width >> srcfmt->p.yuv.xsub2);
                                         ...
                                         unsigned height)
{
	...
	conversion_handler_t *func = NULL;
	const zbar_format_def_t *srcfmt = NULL;
    const zbar_format_def_t *dstfmt = NULL;
                       ...
                       const uint32_t *dsts)
{
    ...
    zprintf(8, "from %.4s(%08" ") to", (char*)&src, src);
...
int zbar_negotiate_format (zbar_video_t *vdo,
                           zbar_window_t *win)
{
	errinfo_t *errdst = NULL;
    static const uint32_t y800[2] = { fourcc('Y','8','0','0'), 0 };
    const uint32_t *srcs = NULL;
    const uint32_t *dsts = NULL;
