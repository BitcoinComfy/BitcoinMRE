...
static int _zbar_mutex_init (zbar_mutex_t *lock)
{
    lock->count = 1;
...
static void _zbar_mutex_destroy (zbar_mutex_t *lock)
{
    DeleteCriticalSection(&lock->mutex);
}

static int _zbar_mutex_lock (zbar_mutex_t *lock)
{
...
static int _zbar_mutex_unlock (zbar_mutex_t *lock)
{
    if(lock->count-- <= 1)
        assert(0);
